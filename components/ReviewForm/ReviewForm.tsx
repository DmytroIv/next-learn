  ...
  return (
    <form onSubmit={ handleSubmit(onSubmit) }>
      ...
      { isSuccess && <div className={ styles.success }>
        ...
        <Button tabIndex={isOpened ? 0 : -1} onClick={() => setIsSuccess(false)} appearance="ghost" className={ styles.close }>⤫</Button>
      </div> }
      { error && <div className={ styles.error }>
        <div>{ error }</div>
        <Button tabIndex={isOpened ? 0 : -1} onClick={() => setError(undefined)} appearance="ghost" className={ styles.close }>⤫</Button>
      </div> }
    </form>
  );
};
